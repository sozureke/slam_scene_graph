...
SemanticGraphNode::SemanticGraphNode()
    ...
    // Инициализация обработчика облаков
   cloud_handler_ = std::make_shared<CloudHandler>(semantic_graph_, cluster_radius_, min_points_per_cluster_);

    RCLCPP_INFO(this->get_logger(), "SemanticGraphNode initialized with max_radius: %f, cluster_radius: %f, min_points_per_cluster: %d", max_radius_, cluster_radius_, min_points_per_cluster_);
}
